...
/** Generic proxy with query-string forwarding */
...
app.get("/api/accounts/:id", (req, res) => {
  req.query = req.query || {}; // ensure query object exists
  proxy("GET", `/accounts/${encodeURIComponent(req.params.id)}`, req, res);
});
app.patch("/api/accounts/:id", (req, res) =>
  proxy("PATCH", `/accounts/${encodeURIComponent(req.params.id)}`, req, res)
);
app.delete("/api/accounts/:id", (req, res) =>
  proxy("DELETE", `/accounts/${encodeURIComponent(req.params.id)}`, req, res)
);

// Simple health (local only)
